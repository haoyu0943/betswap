...
public class TmDateUtil {
	...
	public static String getLastDay() {
		...
		Calendar ca = Calendar.getInstance();
		ca.set(Calendar.DAY_OF_MONTH, ca.getActualMaximum(Calendar.DAY_OF_MONTH));
		String str =date2Str(ca.getTime(), "yyyy-MM-dd");
		return str;
	}
	...
	public static String getCurrYearFirst(){
		...
		Date rd= getYearFirst(currentYear);
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		return sdf.format(rd)+" 00:00:00";
	...
	public static String  getCurrYearLast(){
		Calendar currCal=Calendar.getInstance();
		int currentYear = currCal.get(Calendar.YEAR);
		Date rd=getYearLast(currentYear);
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		return sdf.format(rd)+" 23:59:59";
	}
	 ...
	 * @return Date
	 */
	public static String getMonth(int intermonth){
		SimpleDateFormat format = new SimpleDateFormat("yyyy年MM月");
		Calendar c = Calendar.getInstance();
		c.setTime(new Date());
		c.add(Calendar.MONTH, intermonth);
		Date m = c.getTime();
		return format.format(m);
	}
	/**
	 * 输出一个时间区间内的所有日期，时间粒度取决于String scale。比如：["2021-07-30", "2021-07-31", "2021-08-01", "2021-08-02", "2021-08-03", "2021-08-04"]
	 * @return Date
	 */
	public static List<String> TimeSeqInRange(Date dBegin,Date dEnd,SimpleDateFormat sdf,int unit) throws ParseException {
		...
		while (dEnd.after(calBegin.getTime())) {
			TimeStrList.add(sdf.format(calBegin.getTime()));
			calBegin.add(unit, 1);
		}
		TimeStrList.add(sdf.format(calEnd.getTime()));
		return TimeStrList;
	}
