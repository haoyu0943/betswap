        ...
        <div id="page-wrapper">
            <div id="tabsF" class="ejdhBox">
                <ul class="ejcdUl">
                    ...
                    <li><a href="<%=ctxPath%>/statistic/MovieStatisitc" class="active-li"><lan set-lan="html:stat.movie">电影理财统计</lan></a></li>
...
<script>
   ...
   function checkvalid(){
       ...
       var diff = 0;
       diff = (new Date(edate.substr(0,10).replace("-","/")) - new Date(sdate.substr(0,10).replace("-","/")))/(1000 * 60 * 60 * 24);
       if(diff>=0){
           if (unit === 1) {
               if (diff > 3650) {
   ...
   function dostat(){
       ...
       $.ajax({
           type:"post",
           url:"<%=ctxPath%>/statistic/moviestat",
           data:{
               "sdate":$("#sdate").val(),
               "edate":$("#edate").val(),
               "tjld":$("#tjld").val()
           ...
           success:function(res) {
               ...
               for(var i=0;i<list.length;i++){
                   ...
                   shtml+="<td class=\"compare-label\">"+list[i].fiat+"</td>";
                   shtml+="<td class=\"compare-label\">"+list[i].usdt+"</td>";
                   shtml+="<tr>"
               }
