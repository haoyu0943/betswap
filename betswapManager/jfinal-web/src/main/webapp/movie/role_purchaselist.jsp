...
<head>
    ...
    <title>role of purchase list</title>
...
<body>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover" id="infoTable">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 200px"><lan set-lan="html:moviepreselllist.starttime">开始时间</lan></th>
                                    <th style="width: 260px"><lan set-lan="html:moviepreselllist.endtime">结束时间</lan></th>
    ...
    <script>
        ...
        var status = false;
        function qryPurchaseRoles(){
            clearTimeout(searchTimer);
            searchTimer = setTimeout(function(){
                $.ajax({
                    type:"post",
                    url:"<%=ctxPath%>/movie/qryPurchaseRoles",
                    beforeSend:function(){},
                    ...
                    success:function(data){
                        ...
                        for(var i=0;i<list.length;i++){
                            ...
                            var stadustr="";
                            if(list[i].stadu=="1") {
                                btnstr = "<button type=\"button\" class=\"btn btn-xs btn-warning mr-xs\" onclick='updPurchaserole(this)' data-id='" + list[i].id + "'>"+get_lan("button.modify")+"</button>";
                                btnstr = btnstr + "<button type=\"button\" class=\"btn btn-xs btn-danger\" onclick='delPurchaserole(this)' data-id='" + list[i].id + "'>"+get_lan("button.delete")+"</button>";
                            ...
                            tableRowsArray = [
                                i+1,
                                $.tmDate._toString(list[i].start_date,'yyyy-MM-dd HH'),
                                $.tmDate._toString(list[i].end_date,'yyyy-MM-dd HH'),
        ...
        function delPurchaserole(obj){
            ...
            yyConfirm({title:get_lan("message.prompttitle"),sureText:get_lan("message.sureText"),cancelText:get_lan("message.cancelText"),flag:"1",style:"q",content:get_lan("head.delprop"),callback:function(ok){
                    if(ok){
                        $.ajax({
                            type:"post",
                            url:"<%=ctxPath%>/movie/delPurchaseroleById",
                            data:{"id":id},
                            success:function(data){
                                if(data == true){
        ...
        function updPurchaserole(obj) {
            var id = $(obj).data("id");
            document.location.href="<%=ctxPath%>/movie/newOreditPurchase?movieId=${movieId}&id="+id;
        }

        function sendnew(){
            document.location.href="<%=ctxPath%>/movie/newOreditPurchase?movieId=${movieId}";
        }
