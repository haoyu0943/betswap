...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="page-inner">
                ...
                <div class="HBox">
                     <span class="wwmc color-danger"><lan set-lan="html:msginput.message.subtitle">小标题</lan></span>
                     <input class="form-control" type="text" id="ejbt" style="display: inline-block;width: 30%;" maxlength=60 value="${msg.subtitle}">
                </div>
                ...
                <div class="HBox">
                    ...
                    <select class="form-control" id="lx" style="display: inline-block;width: 150px">
                        <option set-lan="html:articleinput.local" value="0">本地录入</option>
                        <option set-lan="html:articleinput.external" value="1">外部链接</option>
                    </select>
                </div>
                <div class="HBox">
                ...
                <div class="center">
                    <button type="button" class="btn btn-sm btn-success" onclick="saveArticle()" lay-submit="" lay-filter="savebtn"><lan set-lan="html:button.save">保存</lan></button>
    ...
    $(function(){
            ...
            <c:if test="${not empty msg.id}">
               $("#finalImg").attr("src",serviceUrl+"${msg.cover}");
           </c:if>
        var type="${msg.type_flag}";
        if(type==0){
            $("#lx").val("0")
            //$("#xxnr").show();
            $("#fwbk").show();
            //$('.summernote').summernote('code', "${msg.content}");
            $("#weburl").hide();
            //查询内容
            $.ajax({
                type:"post",
                url:"<%=ctxPath%>/system/qryarticlecontent",
                data:{"id":"${id}"},
    ...
    function reback(){
       document.location.href="<%=ctxPath%>/system/articlelist";
    }
    ...
    function saveArticle(){
        formcheckMode.on('submit(savebtn)', function (data) {
            ...
            if(lx==0){
                ...
                formData.append("content",xxnr);
                formData.append("webUrl",null);
            }else{
                var weburl = $("#weburl").val();
                if(isEmpty(weburl)){
                    yyAlert({title:get_lan("message.prompttitle"),sureText:get_lan("message.sureText"),style:"s",content:get_lan("msginput.message.weburl")+get_lan("message.blank")});
                    return;
                }
			...
			// if(isEmpty(ejbt)){
			// 	yyAlert({title:get_lan("message.prompttitle"),sureText:get_lan("message.sureText"),style:"s",content:get_lan("msginput.message.subtitle")+get_lan("message.blank")});
			// 	return;
			// }
            ...
            formData.append("title",xxbt);
            formData.append("subtitle",ejbt);
            formData.append("typeFlag",lx);
            formData.append("userId","${user.user_id}");
            $.ajax({
                type:"post",
                url:serviceUrl+'/system/saveArticle',
                beforeSend:function(){},
                data:formData,
