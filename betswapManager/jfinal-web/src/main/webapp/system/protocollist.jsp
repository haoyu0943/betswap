...
<body>
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            ...
            <div class="page-inner">
                    ...
                    <button type="button" class="btn btn-sm btn-success" onclick="qryPageProtocol()"><lan set-lan="html:movielist.query">查询</lan></button>
                    <button type="button" class="btn btn-sm btn-info" onclick="addProtocol()"><lan set-lan="html:button.new">新增</lan></button>
                    <div class="clear"></div>
            </div>
            <div class="page-inner">
                <div class="row">
    ...
    <script>
        ...
        function qryPageProtocol(){
            ...
            var columns =[
                {data: "rownum",render: function(data, type, row, meta) {
                    ...
                    showstr+="<thead>";
                    showstr+="<tr>";
                        showstr+="<th colspan=\"3\" class=\"text-left\" style='padding-left: 5px;padding-right: 5px'>"+get_lan("movielist.order")+"："+row["type_flag"]+"<span class=\"float-right\">"+$.tmDate._toString(row["create_time"],'yyyy-MM-dd HH:mm')+"</span></th>";
                    showstr+="</tr>";
                    showstr+="</thead>";
                    showstr+="<tbody>";
                    showstr+="<tr>";
                    ...
                    showstr+="<td class=\"td-pass\" width='180px'>"+
                             "<button type=\"button\" class=\"btn btn-xs btn-success\" onclick=\"editProtocol('"+row["id"]+"')\">"+get_lan("button.modify")+"</button>&nbsp;"+
                             "</td>";
                    showstr+="</tr>";
                    showstr+="</tbody>";
                    showstr+=" </table>";
            ...
            getTableDataServer("infoTable",ctxPath+"/system/qryPageProtocol",columns,data,false);
        }
        ...
        <%--			$.ajax({--%>
        <%--        		type:"post",--%>
        <%--        		url:"<%=ctxPath%>/system/delProtocol",--%>
        <%--        		data:{"id":id},--%>
        ...
        function addProtocol(){
           	document.location.href="<%=ctxPath%>/system/crtOreditProtocol";
        }

        function editProtocol(id){
            window.location.href = "<%=ctxPath%>/system/crtOreditProtocol?id="+id;
        }
    </script>
</body>
</html>
