...
<div id="wrapper">
    ...
    <div id="page-wrapper">
            <div id="tabsF" class="ejdhBox">
                <ul class="ejcdUl">
                    <li><a href="<%=ctxPath%>/system/dsshoplist"><lan set-lan="html:dsshoplist.pending.shops">待审商铺</lan></a></li>
                    <li><a href="<%=ctxPath%>/system/shoplist" class="active-li"><lan set-lan="html:dsshoplist.approved.shops">已审商铺</lan></a></li>
                    <div class="clear"></div>
                </ul>
            </div>
...
<script>
    ...
    function qryPageDsShop(){
        ...
        var columns =[
            {data: "rownum",render: function(data, type, row, meta) {
                    ...
                    showstr+="<td class=\"td-pass\" width='180px'>"+
                        "<button type=\"button\" class=\"btn btn-xs btn-success\" onclick=\"examine('"+row["record_id"]+"')\">"+get_lan("button.query")+"</button></td>";
                    showstr+="</tr>";
                    showstr+="</tbody>";
                    showstr+=" </table>";
                    return showstr;
        ...
        getTableDataServer("infoTable",ctxPath+"/system/qryPageShop",columns,data,false);
    }


    function examine(recordid){
        var url="<%=ctxPath%>/system/shopDetail?checkflg=2&recordid="+recordid;
