...
//	}
	...
	/**
	 * 获取文件信息
	 * 方法名：getFileAttribute<BR>
	 * 创建人：qiaobo <BR>
	 * 时间：2016年12月22日-上午10:39:17 <BR>
	 * @param fileAndPath void<BR>
	 * @exception <BR>
	 * @since  1.0.0
	 */
	...
	public static File writeFileByBytesArr(byte[] content, String filename){
     ...
     * @param content 追加的内容   
     */    
    public static void appendBytesToFile(String fileName, byte[] content) {   
    ...
    public static byte[] toBytesByFile(File file){
    	byte[] buffer = null;
     ...
     * 方法名：byte2image<BR>
     * 创建人：qiaobo <BR>
     * 时间：2017年4月19日-下午2:56:43 <BR>
     * @param data
     * @param path void<BR>
     * @exception <BR>
     * @since  1.0.0
     */
    public static boolean byteToImage(byte[] data,String path){
        if(data.length<3|| StrKit.isBlank(path)) return false;
        path = path.replace("/", "\\");
        FileImageOutputStream imageOutput = null;
        try{
        	File file = new File(path);
        	File filePath = new File(path.substring(0,path.lastIndexOf(File.separator)));
        	if(!filePath.exists()){
        		filePath.mkdirs();
        	}
        	if(!file.exists()){
    	        ...
    	        System.out.println("图片创建完成 " + path);
        	}
        	return true;
        } catch(Exception ex) {
