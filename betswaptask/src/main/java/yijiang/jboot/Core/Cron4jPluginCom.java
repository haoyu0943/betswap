 ...
 */
public class Cron4jPluginCom implements IPlugin {

    private List<TaskInfo> taskInfoList = new ArrayList<TaskInfo>();
    public static final String defaultConfigName = "cron4j";

    public Cron4jPluginCom() {

    }

    public Cron4jPluginCom(String configFile) {
        this(new Prop(configFile), defaultConfigName);
    }

    public Cron4jPluginCom(Prop configProp) {
        this(configProp, defaultConfigName);
    }

    public Cron4jPluginCom(String configFile, String configName) {
        this(new Prop(configFile), configName);
    }

    public Cron4jPluginCom(Prop configProp, String configName) {
        try {
    ...
    public Cron4jPluginCom addTask(String cron, Runnable task, boolean daemon, boolean enable) {
        taskInfoList.add(new TaskInfo(cron, task, daemon, enable));
        return this;
    }

    public Cron4jPluginCom addTask(String cron, Runnable task, boolean daemon) {
        return addTask(cron, task, daemon, true);
    }

    public Cron4jPluginCom addTask(String cron, Runnable task) {
        return addTask(cron, task, true, true);
    }

    public Cron4jPluginCom addTask(String cron, ProcessTask processTask, boolean daemon, boolean enable) {
        taskInfoList.add(new TaskInfo(cron, processTask, daemon, enable));
        return this;
    }

    public Cron4jPluginCom addTask(String cron, ProcessTask processTask, boolean daemon) {
        return addTask(cron, processTask, daemon, true);
    }

    public Cron4jPluginCom addTask(String cron, ProcessTask processTask) {
        return addTask(cron, processTask, true, true);
    }

    public Cron4jPluginCom addTask(String cron, Task task, boolean daemon, boolean enable) {
        taskInfoList.add(new TaskInfo(cron, task, daemon, enable));
        return this;
    }

    public Cron4jPluginCom addTask(String cron, Task task, boolean daemon) {
        return addTask(cron, task, daemon, true);
    }

    public Cron4jPluginCom addTask(String cron, Task task) {
        return addTask(cron, task, true, true);
    }
